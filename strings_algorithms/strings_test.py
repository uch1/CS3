...
class StringsTest(unittest.TestCase):
    ...
    def test_contains_with_matching_patterns(self):
        ...
        assert contains('zzz', 'zz') is True
        assert contains('qqq', 'qq') is True
        assert contains('dbc', 'dbc') is True
        assert contains('uuu', 'u') is True
    ...
    def test_contains_with_non_matching_patterns(self):
        ...
        assert contains('abd', 'c') is False
        assert contains('sap', 'pc') is False
    ...
    def test_contains_with_complex_patterns(self):
        ...
        assert contains('corporate', 'rate') is True
        assert contains('foster', 'ter') is True
        assert contains('mother', 'ther') is True
    ...
    def test_find_index_with_matching_patterns(self):
        ...
        assert find_index('zzz', 'zz') == 0
        assert find_index('bba', 'a') == 2
        assert find_index('ike', 'k') == 1
        assert find_index('bigbluewhales', 's') == 12
    ...
    def test_find_index_with_non_matching_patterns(self):
        ...
        assert find_index('applesandoranges', 'zc') is None
        assert find_index('veganapples', 'wc') is None
        assert find_index('makeschool', 'tx') is None
        assert find_index('majorleaguehacking', 'vg') is None
        assert find_index('kitura', 'nu') is None
